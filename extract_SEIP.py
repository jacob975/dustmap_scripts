...
Usage:
    extract_SEIP.py [SEIP table]
Output:
    1. The SED of sources in JHK(UKIDSS), IRAC, and MIPS 
    2. The SED of sources in JHK(2MASS), IRAC, and MIPS 
    3. The fake coordinate of sources.
    4. The fake Av
    5. The fake quality label
Editor:
    Jacob975
...
20190814 version alpha 1:
    1. The code works.
20200416 version alpha 2:
    1. Add a new output of SED, using 2MASS standard.
'''
...
if __name__ == "__main__":
    ...
    if len(argv) != 2:
        ...
        print ("extract_SEIP.py [SEIP table]")
        exit()
    ...
    TWOMASS_SPITZER_flux_sed = np.array(np.transpose([  
        J2flux[:,0],
        H2flux[:,0],
        K2flux[:,0],
        IR1flux[:,0],
    ...
    UKIDSS_SPITZER_flux_sed = np.array(np.transpose([  
        JUflux,
        HUflux,
        KUflux,
        IR1flux[:,0],
