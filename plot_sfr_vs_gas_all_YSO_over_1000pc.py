...
if __name__ == "__main__":
    ...
    index_over1000pc_class_I = distance_class_I > 1000
    print(index_over1000pc_class_I)
    ...
    index_over1000pc_class_F = distance_class_F > 1000
    #--------------------------------------------
    # Plot the figure
    fig, ax = plt.subplots(figsize = (8,8))
    #----------
    # Class_I 
    if len(gas_sigma_class_I[(~index_U_class_I) & (index_over1000pc_class_I)]) != 0:
        ax.errorbar(
            x = gas_sigma_class_I[(~index_U_class_I) & (index_over1000pc_class_I)],
            xerr = e_gas_sigma_class_I[(~index_U_class_I) & (index_over1000pc_class_I)],
            y = sfr_sigma_class_I[(~index_U_class_I) & (index_over1000pc_class_I)],
            yerr = e_sfr_sigma_class_I[(~index_U_class_I) & (index_over1000pc_class_I)],
            label= 'Class I YSO',
            color = 'b',
            fmt = 'o'
        )
    ...
    #----------
    # Class_F
    if len(gas_sigma_class_F[(~index_U_class_F) & (index_over1000pc_class_F)]) != 0:
        ax.errorbar(
            x = gas_sigma_class_F[(~index_U_class_F) & (index_over1000pc_class_F)],
            xerr = e_gas_sigma_class_F[(~index_U_class_F) & index_over1000pc_class_F],
            y = sfr_sigma_class_F[(~index_U_class_F) & (index_over1000pc_class_F)],
            yerr = e_sfr_sigma_class_F[(~index_U_class_F) & (index_over1000pc_class_F)],
    ...
    # power law
    gas_sigma_tot = np.hstack((
        gas_sigma_class_I[(~index_U_class_I) & (index_over1000pc_class_I)],
        gas_sigma_class_F[(~index_U_class_F) & (index_over1000pc_class_F)]
    ))
    e_gas_sigma_tot = np.hstack((
        e_gas_sigma_class_I[(~index_U_class_I) & (index_over1000pc_class_I)],
        e_gas_sigma_class_F[(~index_U_class_F) & (index_over1000pc_class_F)]
    ))
    sfr_sigma_tot = np.hstack((
        sfr_sigma_class_I[(~index_U_class_I) & (index_over1000pc_class_I)],
        sfr_sigma_class_F[(~index_U_class_F) & (index_over1000pc_class_F)]
    ))
    e_sfr_sigma_tot = np.hstack((
        e_sfr_sigma_class_I[(~index_U_class_I) & (index_over1000pc_class_I)],
        e_sfr_sigma_class_F[(~index_U_class_F) & (index_over1000pc_class_F)]
    ))
    print (gas_sigma_tot)
    print (e_gas_sigma_tot)
    print (sfr_sigma_tot)
    print (e_sfr_sigma_tot)

    log_gas_sigma_tot = np.log10(gas_sigma_tot)
    e_log_gas_sigma_tot = np.log10(np.divide(gas_sigma_tot+e_gas_sigma_tot, gas_sigma_tot))
    log_sfr_sigma_tot = np.log10(sfr_sigma_tot)
    e_log_sfr_sigma_tot = np.log10(np.divide(sfr_sigma_tot+e_sfr_sigma_tot, sfr_sigma_tot))
    
    popt = np.polyfit(log_gas_sigma_tot, log_sfr_sigma_tot, 1, w = e_log_gas_sigma_tot)

    #popt, pcov = curve_fit(power_law, gas_sigma_tot, sfr_sigma_tot)
    """
    print(popt)
    #print(pcov)
    p = np.poly1d(popt)
    x_samples = np.linspace(0,3,10)
    ax.plot(
        np.power(10, x_samples),
        np.power(10, p(x_samples)),
        color = 'b',
        label = 'fitting result with slope = {0}'.format(popt[0])
    )
    """
    ...
    ax.legend()
    fig.savefig("chiu20_sfr_vs_gas_Av_regions_over_1000pc.png")
    #-----------------------------------
    # Measure time
