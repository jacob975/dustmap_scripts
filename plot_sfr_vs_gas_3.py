...
if __name__ == "__main__":
    ...
    # Each single cloud
    cloud_data = load2py_mq_cloud(col_list)
    cloud_data = np.array(cloud_data, dtype=float)
    gas_sigma_cloud = cloud_data[:,0]
    e_gas_sigma_cloud = cloud_data[:,1]
    sfr_sigma_cloud = cloud_data[:,2]
    e_sfr_sigma_cloud = cloud_data[:,3]
    # Av regions
    region_data = load2py_mq_av_region(col_list)
    region_data = np.array(region_data, dtype=float)
    gas_sigma_Av_regions = region_data[:,0]
    e_gas_sigma_Av_regions = region_data[:,1]
    sfr_sigma_Av_regions = region_data[:,2]
    e_sfr_sigma_Av_regions = region_data[:,3]
